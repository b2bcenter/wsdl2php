...
class PhpSoapConverter extends SoapConverter
{
    ...
    private function visitMessage(OperationMessage $message, $hint, \GoetasWebservices\XML\SOAPReader\Soap\Operation $operation, Service $service)
    {
        if (!isset($this->classes['__' . spl_object_hash($message)])) {
            ...
            list ($name, $ns) = $this->findPHPName($message, static::inflector()->classify($hint));
            $bodyClass->setName(static::inflector()->classify($name));
            ...
            $envelopeClass->setName(static::inflector()->classify($name));
            ...
            $headerClass->setName(static::inflector()->classify($name));
    ...
    private function visitMessageParts(PHPClass $class, array $parts)
    {
        ...
        foreach ($parts as $part) {
            ...
            if ($part->getElement()) {
                $property->setName(static::inflector()->camelize($part->getElement()->getName()));
                $property->setType($this->converter->visitElementDef($part->getElement()));
            } else {
                $property->setName(static::inflector()->camelize($part->getName()));
